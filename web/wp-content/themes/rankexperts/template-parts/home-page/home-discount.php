<?php if (get_field('services_block_display', get_the_ID())): ?>
    <section class="discount">
        <div class='container'>
            <?php if ($discountTitle = get_field('discount__title', get_the_ID())): ?>
                <h2 class="discount__title title">
                    <?php echo $discountTitle; ?>
                    <?php if ($discountSubTitle = get_field('discount__subtitle', get_the_ID())): ?>
                        <span>
                            <?php echo $discountSubTitle; ?>
                        </span>
                    <?php endif; ?>
                </h2>
            <?php endif; ?>
            <div class="cards">
                <?php $cardsDiscount = get_field('discount__posts', get_the_ID()); ?>
                <?php if (isset($cardsDiscount)): ?>
                    <?php foreach ($cardsDiscount as $card): ?>
                        <?php if (get_field('add_discount', $card->ID)): ?>
                            <article class="card-item">
                                <div class="card-item__inner">
                                    ...
                                    <?php if (!empty(get_field('add_discount', $card->ID))): ?>
                                        <!-- Discount-->
                                        <div class="card-item__discount"><span>
                                                <?php echo get_field('add_discount', $card->ID); ?>
                                            </span>%</div>
                                        <!-- Discount-->
                                    <?php endif; ?>
                                </div>
                                <?php if (!empty($card->post_excerpt)): ?>
                                    <p class="card-item__text">
                                        <?php echo $card->post_excerpt; ?>
                                    </p>
                                <?php endif; ?>
                                <div class="card-item__link-box">
                                    <a class="card-item__link" href="<?php echo $card->guid; ?>">Learn more
