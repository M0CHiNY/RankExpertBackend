<?php if (get_field('services__display', get_the_ID())): ?>
  <section class="marketing" id="down">
    <div class="container">
      <?php if ($divisionTitle = get_field('division_title', get_the_ID())): ?>
        <h2 class="marketing__title title">
          <?php echo $divisionTitle; ?>
        </h2>
      <?php endif; ?>
      <?php if ($slogan = get_field('division_slogan', get_the_ID())): ?>
        <p class="marketing__slogan">
          <?php echo $slogan ?>
        </p>
      <?php endif; ?>
      <div class="marketing__wrap">
        <?php if (get_field('settings_for_categories', 'option')): ?>
          <?php foreach (get_field('settings_for_categories', 'option') as $settings): ?>
            <div class="marketing__card-box">
              <?php if (!empty($settings['settings__title'])): ?>
                <h3 class="marketing__card-title">
                  <?php echo $settings['settings__title']; ?>
                </h3>
              <?php endif; ?>
              <?php if (!empty($settings['settings__description'])): ?>
                <p class="marketing__card-descr">
                  <?php echo $settings['settings__description']; ?>
                </p>
              <?php endif; ?>
              <?php ?>
              <div class="cards">
                ...
                <?php if ($ServicesPost->have_posts()):
                  while ($ServicesPost->have_posts()):
                    ...
                    ?>
                    <?php if (get_field('add_to_home', get_the_ID())): ?>
                      <article class="card-item">
                        ...
                        <div class="card-item__link-box">
                          <a class="card-item__link" href="<?php echo get_permalink(); ?>">Learn more
        ...
        <?php if ($btn = get_field('button_name', 'option')): ?>
          <div class="marketing__btn-inner">
            <a class="marketing__btn btn" href="<?php echo get_field('button_link', 'option') ?>">
              <?php echo $btn; ?>
